...
let
  ...
  nixClawdbot = inputs.nix-clawdbot;
  toolSets = import "${nixClawdbot}/nix/tools/extended.nix" ({ inherit pkgs; } // toolOverrides);
  defaultPackage =
    if toolOverridesEnabled && cfg.package == pkgs.clawdbot then
      (pkgs.clawdbotPackages.withTools toolOverrides).clawdbot
    else
      cfg.package;
  appPackage = if cfg.appPackage != null then cfg.appPackage else defaultPackage;
  generatedConfigOptions = import "${nixClawdbot}/nix/generated/clawdbot-config-options.nix" {
    lib = lib;
  };
  ...
  firstPartyPlugins = lib.filter (p: p != null) (
    lib.mapAttrsToList (
      name: source: if (cfg.firstParty.${name}.enable or false) then { inherit source; } else null
    ) firstPartySources
  );
    ...
    { name, config, ... }:
    {
      options = {
        ...
        stateDir = lib.mkOption {
          type = lib.types.str;
          default = if name == "default" then "${homeDir}/.clawdbot" else "${homeDir}/.clawdbot-${name}";
        ...
        logPath = lib.mkOption {
          ...
          default =
            if name == "default" then
              ...
              "/tmp/clawdbot/clawdbot-gateway-${name}.log";
        ...
        launchd.label = lib.mkOption {
          ...
          default =
            if name == "default" then
              ...
              "com.steipete.clawdbot.gateway.${name}";
        ...
        systemd.unitName = lib.mkOption {
          type = lib.types.str;
          default = if name == "default" then "clawdbot-gateway" else "clawdbot-gateway-${name}";
  ...
  instanceWorkspaceDirs = lib.mapAttrsToList (
    _: inst: resolvePath inst.workspaceDir
  ) enabledInstances;
  ...
  renderSkill =
    ...
    let
      metadataLine =
        if skill ? clawdbot && skill.clawdbot != null then
          "metadata: ${builtins.toJSON { clawdbot = skill.clawdbot; }}"
        else
          null;
      homepageLine =
        if skill ? homepage && skill.homepage != null then "homepage: ${skill.homepage}" else null;
  ...
  resolvePlugin =
    ...
    let
      ...
      clawdbotPlugin =
        if flake ? clawdbotPlugin then
          flake.clawdbotPlugin
        else
          throw "clawdbotPlugin missing in ${plugin.source}";
  ...
  resolvedPluginsByInstance = lib.mapAttrs (
    ...
    if duplicates == [ ] then
      ...
      lib.warn "programs.clawdbot.instances.${instName}: duplicate plugin names detected (${lib.concatStringsSep ", " duplicates}); last entry wins." ordered
  ) enabledInstances;
    ...
    let
      gatewayPackage =
        if inst.gatewayPath != null then
          pkgs.callPackage "${nixClawdbot}/nix/packages/clawdbot-gateway.nix" {
      ...
      mergedConfig = lib.recursiveUpdate (lib.recursiveUpdate baseConfig (lib.recursiveUpdate (mkTelegramConfig inst) (mkRoutingConfig inst))) inst.configOverrides;
...
in
{
  ...
  config = lib.mkIf (cfg.enable || cfg.instances != { }) {
    ...
    home.file =
      ...
      // (lib.optionalAttrs cfg.reloadScript.enable {
        ".local/bin/clawdbot-reload" = {
          executable = true;
          source = "${nixClawdbot}/nix/modules/home-manager/clawdbot-reload.sh";
        };
      });
    ...
    home.activation.clawdbotDirs = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      ...
      ${lib.optionalString (
        pluginStateDirsAll != [ ]
      ) "/bin/mkdir -p ${lib.concatStringsSep " " pluginStateDirsAll}"}
    '';
    ...
    home.activation.clawdbotAppDefaults =
    ...
    home.activation.clawdbotLaunchdRelink = lib.mkIf pkgs.stdenv.hostPlatform.isDarwin (
      lib.hm.dag.entryAfter [ "linkGeneration" ] ''
        /usr/bin/env bash ${"${nixClawdbot}/nix/modules/home-manager/clawdbot-launchd-relink.sh"}
      ''
    );
